...
import { esc, showToast, closeModals, canEditRatings, canAccessRatings } from './core.js?v=20260906a';
import { PLACEHOLDER_TEAM_IMG } from '../config/config.js?v=20260906a';
...
function calcRank(scores) {
    const total = EQUIPMENT_CATS.reduce((acc, c) => acc + (scores[c.key]||0), 0);
...
export function bindRatings(db, auth, getState) {
    ...
    window.loadRatingsPage = async function() {
        ...
        if (!wrap) return;
        wrap.innerHTML = '<p style="color:var(--text-dim);font-size:13px;padding:20px 0;"><i class="fas fa-spinner fa-spin"></i> Загрузка...</p>';
        ...
        try {
            ...
            if (!cards.length) {
                wrap.innerHTML = adminBtns + `<div style="text-align:center;padding:60px 20px;color:var(--text-dim);">
            ...
            wrap.innerHTML = `<p style="color:#ef4444;padding:16px;">Ошибка: ${e.message}</p>`;
            console.error('loadRatingsPage:', e);
        }
    };
    ...
    window.openRatingDetail = async function(id) {
        ...
        try {
            ...
            const set = (id, val) => { const el = document.getElementById(id); if (el) { if (typeof val === 'string' && val.includes('<')) el.innerHTML=val; else el.textContent=val; } };
            const rdAva = document.getElementById('rd-ava');
            if (rdAva) rdAva.src = c.img||PLACEHOLDER_TEAM_IMG;
            set('rd-name',  c.name);
            set('rd-role',  c.role||'Актёр дубляжа');
            set('rd-rank-badge',  rankBadge(rank));
            set('rd-score-total', `${total} / ${getMaxScore()} (${pct}%)`);
            const noteEl = document.getElementById('rd-equip-note');
            if (noteEl) noteEl.value = c.equipNote||'';

            const tbl = document.getElementById('rd-equip-table');
            if (tbl) {
                ...
                EQUIPMENT_CATS.map(cat => {
                    ...
                    return `<tr style="border-top:1px solid rgba(124,58,237,0.1);">
                        ...
                        <td style="padding:10px 12px;text-align:right;font-weight:800;font-size:14px;color:var(--text);">
                            ${score}<span style="font-size:10px;color:var(--text-dim);font-weight:400;">/20</span></td>
                        ${isAdmin?`<td style="padding:10px 6px;">
            ...
            if (rdSave) { rdSave.style.display = isAdmin ? 'block' : 'none'; rdSave.onclick = () => saveRatingScores(id); }
            if (rdDel)  { rdDel.style.display  = isAdmin ? 'block' : 'none'; rdDel.onclick  = () => deleteRating(id); }

            document.getElementById('m-rating-detail').style.display = 'flex';
        } catch(e) {
            showToast('Ошибка: ' + e.message, 'error');
            console.error('openRatingDetail:', e);
        }
    };

    window.previewRank = function() {
        const scores = {};
        EQUIPMENT_CATS.forEach(cat => {
    ...
    window.openRatingAddModal = function() {
        ['ra-name','ra-role','ra-img','ra-note'].forEach(id => {
    ...
    window.saveRatingNew = async function() {
        ...
        try {
            await addDoc(collection(db,'ratings'), { name, role:role||'Актёр дубляжа', img, equipNote:note, scores:{}, createdAt:Date.now(), teamId:'' });
    ...
    window.openRatingFromTeam = async function() {
        ...
        if (!listEl) return;
        listEl.innerHTML = '<p style="color:var(--text-dim);font-size:13px;text-align:center;padding:16px;"><i class="fas fa-spinner fa-spin"></i> Загрузка...</p>';
        ...
        try {
            ...
            const members = snap.docs
                ...
                .filter(m => {
                    ...
                    return cat.includes('актёр')||cat.includes('дабер')||role.includes('дабер')||role.includes('актёр')||role.includes('дублёр');
                });

            if (!members.length) {
                listEl.innerHTML = '<p style="color:var(--text-dim);font-size:13px;text-align:center;padding:20px;">Нет участников в категории «Актёр дубляжа».</p>';
                return;
            }

            const existSnap = await getDocs(collection(db,'ratings'));
            const existIds  = existSnap.docs.map(d => d.data().teamId).filter(Boolean);

            listEl.innerHTML = members.map(m => {
                const already = existIds.includes(m.id);
                return `<div class="rft-item" style="${already?'opacity:0.5;pointer-events:none;':''}"
                    onclick="addRatingFromMember('${m.id}','${esc(m.name)}','${esc(m.role)}','${esc(m.img||'')}')">
                    <img src="${esc(m.img||PLACEHOLDER_TEAM_IMG)}" style="width:42px;height:42px;border-radius:50%;object-fit:cover;border:1px solid var(--border);"
            ...
            if (listEl) listEl.innerHTML = `<p style="color:#ef4444;font-size:13px;padding:16px;">Ошибка: ${e.message}</p>`;
        }
    };

    window.addRatingFromMember = async function(teamId, name, role, img) {
        try {
            await addDoc(collection(db,'ratings'), { name, role, img, equipNote:'', scores:{}, createdAt:Date.now(), teamId });
