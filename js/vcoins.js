...
import { esc, showToast, closeModals, showVCoinsPopup } from './core.js?v=20260906c';
import { VCOINS_DEFAULT_PRICES } from '../config/config.js?v=20260906c';
...
export function bindVCoins(db, auth, getState) {
    ...
    async function loadLeaderboardInto(elId, field) {
        ...
        if (!el) return;
        try {
            const q = query(collection(_db, 'users'), orderBy(field, 'desc'), limit(5));
            const snap = await getDocs(q);
            if (snap.empty) { el.innerHTML = '<div class="lb-loading">Пока пусто</div>'; return; }
