// ============================================================
// js/users_search.js — Поиск пользователей + стена профиля (V2.3 Optimized)
// ============================================================
...
import { esc, showToast, closeModals } from './core.js?v=20260906a';

let _db, _auth, _getState;
let _searchTimeout = null;
let _usersCache = null; // Кэш всех пользователей для быстрого поиска
...
export function bindUserSearch(db, auth, getState) {
  ...
  window.onFriendSearchInput = function() {
    ...
    _searchTimeout = setTimeout(doSearch, 300); // Сокращаем дебаунс до 300ms
  };
  ...
  async function doSearch() {
    ...
    try {
      // Получаем кэшированный список или загружаем новый
      let allUsers = _usersCache;
      if (!allUsers || Date.now() - _usersCacheTime > CACHE_DURATION) {
      ...
      res.innerHTML = results
        .filter(u => u.id !== myUid) // Исключаем самого себя
        .map(u => `
          <div class="friend-result-card" onclick="openUserProfile('${u.id}')">
            <img src="${esc(u.avatar || 'https://api.dicebear.com/7.x/identicon/svg')}" class="friend-result-ava" onerror="this.src='https://api.dicebear.com/7.x/identicon/svg'">
  ...
  // Инвалидируем кэш при изменении данных пользователя
  window.invalidateUsersCache = function() {
    _usersCache = null;
    _usersCacheTime = 0;
  };
}
...
export function bindProfileWall(db, auth, getState) {
  ...
  window.loadProfileWall = async function(targetUid) {
    ...
    try {
      ...
      const isAdmin = userData?.role === 'admin' || userData?.role === 'proxyadmin' || userData?.role === 'developer';
  ...
  function renderWallPost(p, myUid, isOwner, isAdmin, targetUid) {
    ...
    const postDate = new Date(p.date).toLocaleString('ru', {day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'});
    
    return `
      <div class="wall-post" id="wpost-${p.id}">
        <div class="wall-post-header">
          <img src="${esc(p.authorAva || 'https://api.dicebear.com/7.x/identicon/svg')}" class="wall-post-ava" onclick="openUserProfile('${p.authorUid}')" onerror="this.src='https://api.dicebear.com/7.x/identicon/svg'">
          <div class="wall-post-meta">
            <span class="wall-post-nick" onclick="openUserProfile('${p.authorUid}')">${esc(p.authorNick)}</span>
            <span class="wall-post-date">${postDate}</span>
          </div>
