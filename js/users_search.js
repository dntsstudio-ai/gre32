...
export function bindProfileWall(db, auth, getState) {
    ...
    window.loadProfileWall = async function(targetUid) {
        ...
        try {
            const snap = await getDocs(query(
                collection(_db, `users/${targetUid}/wall`),
        ...
        return `<div class="wall-post" id="wpost-${p.id}">
            <div class="wall-post-header">
                <img src="${esc(p.authorAva||'https://api.dicebear.com/7.x/identicon/svg')}" class="wall-post-ava"
                     onclick="openUserProfile('${p.authorUid}')" onerror="this.src='https://api.dicebear.com/7.x/identicon/svg'">
    ...
    window.previewWallImage = function(input) {
        ...
        reader.onload = e => {
            document.getElementById('wall-img-preview').style.display = 'flex';
            document.getElementById('wall-img-preview-img').src = e.target.result;
    ...
    window.clearWallImage = function() {
        document.getElementById('wall-img-preview').style.display = 'none';
        document.getElementById('wall-img-preview-img').src = '';
    ...
    window.deleteWallPost = async function(targetUid, postId) {
        ...
        const { deleteDoc: del } = await import("https://www.gstatic.com/firebasejs/10.8.1/firebase-firestore.js");
        await del(doc(_db, `users/${targetUid}/wall`, postId));
        showToast('Удалено');
        window.loadProfileWall(targetUid);
    };
