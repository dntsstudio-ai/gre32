...
function getPlayerState(containerId) {
    ...
    if (!el) return {};
    if (!el._swsState) el._swsState = { isDrive:false, isYoutube:false, isTrailer:false, menuOpen:false, controlsTimer:null, autoSkip:false, autoNext:false, episodes:[], currentIdx:0, onNext:null, onSkip:null };
    return el._swsState;
}
export function initPlayer(containerId, options={}) {
    const container = document.getElementById(containerId);
    if (!container) return;
    const { url='', title='', muted=false, episodes=[], currentIdx=0, autoSkip=false, autoNext=false, onNext=null, onSkip=null, isTrailer=false } = options;
    ...
    container.innerHTML = buildHTML(containerId, buildEmbedSrc(url), title, st.isDrive, muted, episodes, currentIdx, isTrailer);
    attachEvents(containerId);
}
function buildEpListHTML(containerId, episodes, currentIdx) {
    if (episodes.length <= 1) return '';
    return `<div class="swsp-menu-section"><div class="swsp-menu-label">Серии</div><div class="swsp-ep-list" id="swsp-ep-list-${containerId}">${episodes.map((ep,i)=>`<button class="swsp-ep-btn ${i===currentIdx?'active':''}" onclick="swspSelectEp('${containerId}',${i})"><span class="swsp-ep-num">${i+1}</span><span class="swsp-ep-name">${ep.name||`Серия ${i+1}`}</span>${i===currentIdx?'<i class="fas fa-play" style="color:var(--accent);font-size:10px;margin-left:auto;"></i>':''}</button>`).join('')}</div></div>`;
}
function buildHTML(containerId, src, title, isDrive, muted, episodes, currentIdx, isTrailer) {
    if (isDrive) {
        return `<div class="swsp" id="swsp-${containerId}"><iframe class="swsp-iframe swsp-iframe--drive" id="swsp-iframe-${containerId}" src="${src}" allow="autoplay;fullscreen;picture-in-picture" allowfullscreen frameborder="0" title="${title}"></iframe>${!isTrailer?`<button class="swsp-skip-btn" id="swsp-skip-${containerId}" style="display:none;"><i class="fas fa-forward"></i> Пропустить заставку</button><button class="swsp-next-btn" id="swsp-next-${containerId}" style="display:none;">Следующая серия <i class="fas fa-step-forward"></i></button><button class="swsp-drive-menu-btn" id="swsp-menu-btn-${containerId}" onclick="swspToggleMenu('${containerId}')" title="Меню"><i class="fas fa-chevron-up"></i></button><div class="swsp-menu" id="swsp-menu-${containerId}"><div class="swsp-menu-header"><span class="swsp-menu-title"><i class="fas fa-sliders-h"></i> Настройки</span><button class="swsp-menu-close" onclick="swspToggleMenu('${containerId}')"><i class="fas fa-chevron-down"></i></button></div><div class="swsp-menu-body"><div class="swsp-menu-section"><div class="swsp-menu-label">Авто-функции</div><label class="swsp-toggle-row"><div class="swsp-toggle-info"><i class="fas fa-forward" style="color:var(--accent)"></i><span>Авто-пропуск заставки</span></div><div class="swsp-toggle" id="swsp-tog-skip-${containerId}" onclick="swspToggleAutoSkip('${containerId}')"><div class="swsp-toggle-knob"></div></div></label><label class="swsp-toggle-row"><div class="swsp-toggle-info"><i class="fas fa-step-forward" style="color:var(--accent)"></i><span>Авто-следующая серия</span></div><div class="swsp-toggle" id="swsp-tog-next-${containerId}" onclick="swspToggleAutoNext('${containerId}')"><div class="swsp-toggle-knob"></div></div></label></div>${buildEpListHTML(containerId,episodes,currentIdx)}</div></div>`:''}`;
    }
    const finalSrc = src + ((!isDrive && muted) ? '&mute=1' : '');
    return `<div class="swsp" id="swsp-${containerId}"><iframe class="swsp-iframe" id="swsp-iframe-${containerId}" src="${finalSrc}" allow="autoplay;fullscreen;picture-in-picture" allowfullscreen frameborder="0" title="${title}"></iframe><div class="swsp-overlay" id="swsp-ov-${containerId}">${title?`<div class="swsp-top"><span class="swsp-title">${title}</span></div>`:''}<div class="swsp-midzone" id="swsp-mid-${containerId}"></div><div class="swsp-controls"><div class="swsp-ctrl-row"><button class="swsp-btn" id="swsp-mute-${containerId}" title="Звук"><i class="fas fa-${muted?'volume-mute':'volume-up'}"></i></button><div style="flex:1"></div>${!isTrailer?`<button class="swsp-btn swsp-menu-btn" id="swsp-menu-btn-${containerId}" title="Настройки" onclick="swspToggleMenu('${containerId}')"><i class="fas fa-chevron-up"></i></button>`:''}<button class="swsp-btn" id="swsp-fs-btn-${containerId}" title="Полный экран"><i class="fas fa-expand" id="swsp-fs-icon-${containerId}"></i></button></div></div>${!isTrailer?`<button class="swsp-skip-btn" id="swsp-skip-${containerId}" style="display:none;"><i class="fas fa-forward"></i> Пропустить заставку</button><button class="swsp-next-btn" id="swsp-next-${containerId}" style="display:none;">Следующая серия <i class="fas fa-step-forward"></i></button>`:''}</div>${!isTrailer?`<div class="swsp-menu" id="swsp-menu-${containerId}"><div class="swsp-menu-header"><span class="swsp-menu-title"><i class="fas fa-sliders-h"></i> Настройки</span><button class="swsp-menu-close" onclick="swspToggleMenu('${containerId}')"><i class="fas fa-chevron-down"></i></button></div><div class="swsp-menu-body"><div class="swsp-menu-section"><div class="swsp-menu-label">Авто-функции</div><label class="swsp-toggle-row"><div class="swsp-toggle-info"><i class="fas fa-forward" style="color:var(--accent)"></i><span>Авто-пропуск заставки</span></div><div class="swsp-toggle" id="swsp-tog-skip-${containerId}" onclick="swspToggleAutoSkip('${containerId}')"><div class="swsp-toggle-knob"></div></div></label><label class="swsp-toggle-row"><div class="swsp-toggle-info"><i class="fas fa-step-forward" style="color:var(--accent)"></i><span>Авто-следующая серия</span></div><div class="swsp-toggle" id="swsp-tog-next-${containerId}" onclick="swspToggleAutoNext('${containerId}')"><div class="swsp-toggle-knob"></div></div></label></div>${buildEpListHTML(containerId,episodes,currentIdx)}</div></div>`:''}`;
}
function attachEvents(containerId) {
    ...
    if (st.isDrive) {
        ...
        attachFullscreen(containerId, root); return;
    }
...
window.swspToggleAutoSkip = (containerId) => { const st=getPlayerState(containerId); const tog=document.getElementById(`swsp-tog-skip-${containerId}`); st.autoSkip=!st.autoSkip; tog?.classList.toggle('swsp-toggle--on',st.autoSkip); if(window._syncPlayerSettings)window._syncPlayerSettings(containerId); };
window.swspToggleAutoNext = (containerId) => { const st=getPlayerState(containerId); const tog=document.getElementById(`swsp-tog-next-${containerId}`); st.autoNext=!st.autoNext; tog?.classList.toggle('swsp-toggle--on',st.autoNext); if(window._syncPlayerSettings)window._syncPlayerSettings(containerId); };
window.swspSelectEp = (containerId, idx) => { swspToggleMenu(containerId); if(window.playEpByIdxGlobal)window.playEpByIdxGlobal(idx); };
...
export function playerUpdateEpisodes(containerId, episodes, currentIdx) {
    ...
    if (!listEl) return;
    listEl.innerHTML = episodes.map((ep,i)=>`<button class="swsp-ep-btn ${i===currentIdx?'active':''}" onclick="swspSelectEp('${containerId}',${i})"><span class="swsp-ep-num">${i+1}</span><span class="swsp-ep-name">${ep.name||`Серия ${i+1}`}</span>${i===currentIdx?'<i class="fas fa-play" style="color:var(--accent);font-size:10px;margin-left:auto;"></i>':''}</button>`).join('');
}
