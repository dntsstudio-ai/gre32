...
export function applyUserUI(userData, isAdmin, hasDubAccess) {
    ...
    if (nickEl && userData.nickColor) nickEl.style.color = userData.nickColor;
    // Prefix
    const prefEl = document.getElementById('u-nick-prefix');
    if (prefEl) { if (userData.activePrefix) { prefEl.textContent='['+userData.activePrefix+']'; prefEl.style.display='inline'; } else prefEl.style.display='none'; }

    renderAchProfile(userData);

    document.getElementById('adm-btn-rel').style.display   = isAdmin ? 'inline-flex' : 'none';
    const adminRolesBtn = document.getElementById('btn-admin-roles');
    if (adminRolesBtn) adminRolesBtn.style.display = isAdmin ? 'inline-flex' : 'none';
    const adminVcoinsBtn = document.getElementById('btn-admin-vcoins');
    if (adminVcoinsBtn) adminVcoinsBtn.style.display = isAdmin ? 'inline-flex' : 'none';
